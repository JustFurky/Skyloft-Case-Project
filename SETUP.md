...
### 3.0 Zenject (DI) kurulumu — ZORUNLU
Bağımlılıklar artık `.Instance` yerine Zenject ile enjekte edilir. Sahne bu olmadan çalışmaz:
1. Hiyerarşiye **GameObject → Zenject → Scene Context** ekle.
2. Bir GameObject'e **`GameInstaller`** (`Assets/Scripts/Infrastructure/GameInstaller.cs`) bileşenini
   ekle ve SceneContext'in **Installers** listesine sürükle.
3. `GameInstaller`, `FromComponentInHierarchy` ile bağladığından şu yöneticiler **sahnede** bulunmalı:
   `DataManager`, `LevelManager`, `EnemySpawner`, `ScoreService`, `AudioManager`, `PauseController`,
   `GameStateManager`. (Bölüm 3.2'deki GameObject'ler.)
4. **Player sahnede yerleşik** olmalı (havuzdan değil) ki Zenject `PlayerController/Shooter/Health`'i
   enjekte etsin.
5. İstisna: havuzlanan nesneler (Enemy/Projectile/VFX) Zenject'le enjekte edilmez; havuz erişimi
   `ObjectPooler.Instance` üzerinden kalır (bilinçli, pragmatik tercih).
...
- **Player Data** — oyuncu can/hareket/arena. `PlayerController` **ve** `PlayerHealth`'e
  **aynı** asset atanır.
- **Health Bar Data** — can barı animasyon süresi + dolu/boş hedef renkleri + billboard →
  `PlayerHealthBar`. (Can azaldıkça fill rengi `lowHealthColor`'a doğru smooth geçer.)
...
- **Projectile Data** — hasar, ömür, hitLayers (`Enemy`), `Hit Vfx` (**VfxSpawn**: pool + forward/up
  offset + faceOrigin) → `Projectile`.
- **Enemy Data** — düşman istatistikleri + `Spawn Vfx` ve `Death Vfx` (**VfxSpawn**: pool + offsetler;
  death up-offset 0.5). VFX PoolId'leri artık prefab'da değil **burada** → `Enemy`.

> **VfxSpawn:** Tüm efekt spawn'ları tek tipte toplandı (`VFX/VfxSpawn`): `{ pool, forwardOffset,
> upOffset, faceOrigin }`. Hit/spawn/death hepsi aynı yapı — offsetler artık dağınık değil.
...
- **Can barı (prefab'a ait):** Player altına `World Space` bir Canvas + içine `Image`
  ...
  `Data` = **Health Bar Data**, `Health` boş bırakılabilir (parent'tan otomatik bulur). Bar,
  `PlayerHealth.OnHealthChanged` olayını dinler; HUD'a referans gerektirmez (SRP). Fill rengi
  tween ile sürüldüğünden Image rengini editörde elle ayarlamana gerek yok. Can biterse oyun
  otomatik kaybedilir.

### 3.5 Enemy prefab'ı (havuzlanır)
- `NavMeshAgent` + `EnemyAI` + `Enemy` (sadece `EnemyData` ata — spawn/death VFX artık
  EnemyData içinde) + `EnemyAnimator` (`Anim Data` = **Enemy Animation Data**)
...
- `Rigidbody` (kinematic, otomatik ayarlanır) + trigger `Collider` + `Projectile`
  ...
  Projectile Data asset'inde (hitLayers = `Enemy`, `Hit Vfx.Pool` = **HitVfx PoolId**).
...
- **PauseView** (5. panel, UIController'a bağlanmaz): `CanvasGroup` + `PauseView`
  (Resume/Menu butonları + **Countdown Label** TMP_Text). UIPanel'de **Use Unscaled Time**
